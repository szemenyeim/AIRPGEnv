...
void RPGEnv::Hero::setExplorationMatrix(int rows, int cols)
{
	...
	position.x - 32 <= 0 ? min_x = 0 : min_x = position.y - 32;
	position.x + 32 >= cols ? max_x = cols : max_x = position.y + 32;
...
void RPGEnv::Hero::Exploring(int keypressed)
{
	...
	case (int)'w':
	{
		...
		{
			if (ExplorationMatrix[max_y][col] == 0)
			{
				ExplorationMatrix[max_y][col] = 1;
				experience += 0.1;
			}
		}
		break;
	...
	case (int)'d':
	{
		...
		{
			if (ExplorationMatrix[row][max_x] == 0)
			{
				ExplorationMatrix[row][max_x] = 1;
				experience += 0.1;
			}
		}
